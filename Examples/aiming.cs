...
const string LIDAR_BLOCK_TAG = "Dridar";                 //Тег камер для использования их в качестве ГСН
const string AIM_POINT_BLOCK_TAG = "Кресло";      //Тег блока используемого для ориентира. Если не написано,скрипт попытается его найти сам
const string GYROSCOPE_NAME_TAG = "Гироскоп";     //Тег гироскопов для прицеливания. Это используется если гироскопы на корабле не найдены
const string STATUS_DISPLAY_PANEL_TAG = "ST_STATUS";    //Тег текстовой панели для вывода информации о работе скрипта (статус АСЦ)

const string REMOTE_CONTROL_NAME = "Блок ДУ";       //Название блока удалённого управления для получения информации

const string TARGET_GPS_PANEL = "R_TARGET";             //Название тектовой панели для вывода информации о цели

const string LOCKED_TRIGGER_BLOCK = "R_ALERT";         //Название блока,который будет активирован при обнаружении цели
const string LOCKED_TRIGGER_ACTION = "PlaySound";       //Действие блока,который был указан выше

const string STATE_DISPLAY_PB_TAG = "Projector Alert Computer"; //Название программируемого блока для использования голографического прицела
const string STATE_DISPLAY_PB_ARG_PREFIX = "SIGNAL:";   //Префикс аргументов, который должен быть передан в State Display PB вместе с номером состояния
    
const string ROCKET_LOCKED_FIRE_TAG = "Rocket Launcher";//Название блока для активации,когда наведение стабильно и в радиусе 800 метров
const string ROCKET_LOCKED_FIRE_ACTION = "ShootOnce";   //Действие с указанным выше блоком

const string GATLING_LOCKED_FIRE_TAG = "Gatling Gun";   //Название блока для активации,когда наведение стабильно и в радиусе 800 метров
const string GATLING_LOCKED_FIRE_ACTION = "ShootOnce";  //Действие с указанным выше блоком
//------------------------------ Projectile Parameters Configuration ------------------------------

double SHOOT_AHEAD_TIME = 0;                            //Перерыв в секундах между выстрелами
double SHOOT_AHEAD_DISTANCE = 0;                        //Дистанция в метрах для стрельбы
// я устал
...
float LIDAR_REFRESH_CALC_FACTOR = 1f;                //Estimated ratio of lidars to utilize

bool excludeFriendly = true;                           //Whether to exclude locking on to friendlies

float fivePointInitialLockDist = 2f;                    //The amount of spread for 5 point raycast for easier initial lock-on. Zero to use normal single beam lock-on

int raycastAheadCount = 2;                              //Raycast ahead of of your initial aiming, to mitigate high ping and gyro sync lag. Zero to use normal single beam lock-on
...
if (mode == 0)
{
sb.Append(" Статус: Ожидание").AppendLine(statusMessage.Length > 0 ? " - " + statusMessage : "");
}
else if (mode == 1)
{
sb.AppendLine(" Статус: Поиск");
}
else if (mode == 2)
{
sb.Append(" Статус: ").AppendLine(subCounter >= AIM_STABILIZE_TICKS ? "Цель захвачена" : "Прицеливается");
}
...
if (subMode == 0)
{
sb.Append(" Тип: ").Append(useOffsetTargeting ? "Часть " : "").Append("Центр ").AppendLine(leadType);
}
else if (subMode == 1)
{
sb.AppendLine(" Тип: Сопровождение цели");
}

sb.Append(" Авто прицеливание: ").AppendLine(isAim ? "ВКЛЮЧЕНО" : "-");
sb.Append(" Авто огонь: ").AppendLine(isFire ? "ВКЛЮЧЕНО" : "-");
...
sb.Append(" Погрешность: ").AppendLine(isUnreachable ? "[X]" : Math.Round(Math.Tan(angleToTarget) * lidarTargetDistance, 2).ToString());
sb.Append(" Наведение: ").AppendLine(lastTargetFound ? "Стабильное" : "Нестабильное");
sb.Append("\n Расстояние до цели: ").AppendLine(Math.Round(lidarTargetDistance, 2).ToString());
sb.Append(" Скорость цели: ").AppendLine(Math.Round(lidarTargetSpeed, 2).ToString());
sb.Append(" Радиус цели: ").AppendLine(Math.Round(lidarTargetRadius, 2).ToString());
...
{
aimLidars = GetLidars();
if (aimLidars == null) throw new Exception("---  Инициализация провалена:Камеры украли цыгане ---");

aimPointBlock = GetAimPointBlock();
if (aimPointBlock == null) throw new Exception("--- Инициализация провалена:Нет ориентира ---");

List<IMyTerminalBlock> gyros = GetGyroscopes();
if (gyros == null) throw new Exception("---  Инициализация провалена:Нет гироскопов ---");
...
if (remoteControl == null) throw new Exception("---  Инициализация провалена:Нет ДУ ---");
...
if (blocks.Count > 1)
{
Echo("Error: Не может быть ориентиров больше одного с этим тегом: " + AIM_POINT_BLOCK_TAG + ". Пожалуйста,переименуйте их.");
return null;
}
if (blocks.Count == 1)
{
...
{
Echo("Error: Не обнаружено гироскопов:Пожалуйста,установите один.");
return null;
}
}
